...
def get_anchor_donations(request):
    ...
    try:
        # 查询主播收到的所有打赏记录
        donations = PaymentExpenditureRecord.objects.filter(
            anchor_id=anchor_id,
        ...
        for donation in donations:
            # 查询关联用户信息
            user = ChatUser.objects.filter(id=donation.user_id).first()
            ...
            donation_data.append({
                ...
                "user_name": user_name,
                "donation_amount": donation.amount,
                "donation_date": donation.payment_date  # 使用 payment_date
            })
...
def make_donation(request):
    ...
    try:
        ...
        amount = request.data.get('amount')  # 打赏金额（余额）

        # 检查参数是否缺失
        if not user_id or not anchor_id or not amount:
            return Response({"code": 1, "message": "Missing required parameters."},
        ...
        try:
            ...
            return Response({"code": 1, "message": "Invalid amount format."},
                            status=status.HTTP_400_BAD_REQUEST)

        if amount <= 0:
            return Response({"code": 1, "message": "Amount must be greater than 0."},
                            status=status.HTTP_400_BAD_REQUEST)

        # 获取用户余额
        user_balance = UserBalance.objects.filter(user_id=user_id).first()
        if not user_balance:
            return Response({"code": 1, "message": "User balance not found."},
        ...
        if user_balance.balance < amount:
            return Response({"code": 1, "message": "Insufficient balance."},
                            status=status.HTTP_400_BAD_REQUEST)
        ...
        if not anchor:
            return Response({"code": 1, "message": "Anchor not found."},
                            status=status.HTTP_404_NOT_FOUND)
        ...
        return Response({
            ...
            "data": {
                "donation_id": expenditure_record.id,
                "user_name": user_name,
                "anchor_name": anchor_name,
                "amount": amount,
                "donation_date": expenditure_record.payment_date,  # 使用 payment_date
